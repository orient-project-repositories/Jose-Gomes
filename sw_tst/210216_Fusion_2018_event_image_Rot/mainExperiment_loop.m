...
%     xR = xR+vR*dt;
    chiR = state2chi(RotR,PosAmersR);
    S_R = rukfPropagation(dt,chiR,chiAntR,omega_bR,S_R,omega_i,...
        Qc,g);
...
%     chiI = state2chi(RotI,vI,xI,PosAmersI);
    ...
    if obsTimes(i) == 1
        ...
        [y,yAmers,trackerMain,trackerBis,pointsMain,validityMain,...
            ...
            fileImages,ParamFilter,RotR,PosAmersR,i,S_R,myTracks);
        
        % update state
        param.yAmers = yAmers;
        [chiR,omega_bR,S_R] = rukfUpdate(chiR,omega_bR,...
           S_R,y,param,R,ParamFilter);
        [RotR,PosAmersR] = chi2state(chiR);
...
%         end
        ...
        trajR = updateTraj(trajR,RotR,omega_bR,i);
%         trajL = updateTraj(trajL,RotL,vL,xL,omega_bL,a_bL,i);
%         trajRef = updateTraj(trajRef,RotRef,vRef,xRef,omega_bRef,a_bRef,i);
%         trajU = updateTraj(trajU,RotU,vU,xU,omega_bU,a_bU,i);
%         trajI = updateTraj(trajI,RotI,vI,xI,omega_bI,a_bI,i);
