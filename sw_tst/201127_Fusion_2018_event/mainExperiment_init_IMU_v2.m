...
% tracks = readmatrix( './tracks.txt') ;
tracks = readmatrix( neurocams3_data( [], 'tracks.txt') );

init_time = tracks(1,2);
for i = 1:length(tracks(:,2))
   tracks(i,2) = tracks(i,2) - init_time;
end
% omega = IMU(tMin:end,5:7)'; 
% acc = IMU(tMin:end,2:4)'; 

pre_omega = IMU(tMin:end,5:7)'; 
pre_acc = IMU(tMin:end,2:4)'; 

omega = [-pre_omega(1,:);-pre_omega(3,:);-pre_omega(2,:)];
acc = [-pre_acc(1,:);-pre_acc(3,:);-pre_acc(2,:)];
...
ParamGlobal.tracks = tracks(idx,:);
ParamGlobal.tracks_or = tracks(idx,:);   
ParamGlobal.tracks(:,1) = ParamGlobal.tracks(:,1) + 1;
ParamGlobal.tracks_or(:,1) = ParamGlobal.tracks_or(:,1) + 1;
