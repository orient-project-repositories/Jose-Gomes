function [orb_slam, trajs, i] = mainExperiment_loop(orb_slam, IMU_img_struct, state_camera_struct, NbStepsMax, ParamFilter, ParamGlobal)
...
feature_table.latest = ParamGlobal.Slice.tEnd;
feature_table.per = ParamGlobal.PerCam;
feature_table.features = [0,1,2,3];
...
for i = 2:NbStepsMax
    ...
    if obsTimes == 1
        ...
        if length(feature_table.features(:, 1)) ~=1
            ParamGlobal.Slice.slice = feature_table.features(2:end, :);
            feature_table.features(2:end, :) = [];
        else
            ParamGlobal.Slice.slice = [];
        end
        
        ParamGlobal.Slice.tStart = feature_table.latest;
        feature_table.latest = feature_table.latest + feature_table.per;
        ParamGlobal.Slice.tEnd = feature_table.latest;
...
%         setPoints(trackerMain,pointsMain,validityMain);
    ...
    helper_msg = (RotR)' * trajR.v(:,1);
    
%     twist_msg.Vx = - trajR.v(1,i);
%     twist_msg.Vy = trajR.v(3,i);
%     twist_msg.Vz = trajR.v(2,i);
    twist_msg.Vx = - helper_msg(1);
    twist_msg.Vy = helper_msg(3);
    twist_msg.Vz = helper_msg(2);
...
function updateFeaturesCall(~, msg)
    ...
    feature_table.features(end+1,1) = msg.Id +1;
    feature_table.features(end,2) = msg.Ts.Sec + msg.Ts.Nsec * 10^-9;
    feature_table.features(end,3) = msg.X;
    feature_table.features(end,4) = msg.Y;
    
    if (msg.Ts.Sec + msg.Ts.Nsec * 10^-9) > (feature_table.latest + feature_table.per)
        obsTimes = 1;
    end
end
